    ...
    def execute_query(self, query):
        ...
        with self.get_cursor() as cursor:
            try:
                ...
                logger.info(f"查询已成功执行: {query}")
            except Error as e:
                self.connection.rollback()
                logger.error(f"执行查询时出现错误: {e}; 查询: {query}")
                logger.error("事务已回滚")
                raise
    ...
    def execute_read_query(self, query):
        ...
        with self.get_cursor() as cursor:
            try:
                ...
                logger.error(f"读取查询时出现错误: {e}; 查询: {query}")
                raise
...
if __name__ == "__main__":
    ...
    # 执行语句
    update_query = """
        UPDATE xfcy_inspection_sub_issues
        SET opinion = '查验情况',
        opinion_type = 0
        WHERE
    	inspection_project_item_id = '4b8b14977c0146d7aa6491ee1c9b7d54' 
    	AND opinion_type IS NULL
        """
    try:
